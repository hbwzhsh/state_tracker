...
from keras.layers.core import Dense, SpatialDropout1D, Lambda
...
import numpy as np
import pickle
from data_loader.load_CamRest import get_X_Y_from_raw_text, slot_state_dict, dict_to_feature
from dialogue_state.domain import small_intent_for_kvret,get_valid_action_from_intent
...
from dialogue_state.reader import load_config,auto_type_set
feature_names = ["intent","keyword","lastact","slotfilling_state"]
useful_slots = ['time', 'distance', 'event', 'party',  'location', 'date', 'agenda', 'poi_type', 'address', 'room', 'poi', 'weather_attribute']
num_slots = len(useful_slots)
class ClassModel():
    def __init__(self):
    ...
    def show_paras(self):
        for att in ["hidden_size","batch_size","MAX_SENTENCE_LENGTH","turn_number"]:
            print(att,self.__dict__.get(att))
            print("paras")
    # keras
    @staticmethod
    def get_freq_word(top_n):
        import nltk
...
class ActionModel(ClassModel):
    ...
    def evaluate(self, Xtest, ytest):
        ...
        for i in range(len(self.all_action)):
            if i not in index_y_hat and i not in index_y:
                index_y_hat.append(i)
                index_y.append(i)
        index_y_hat = np.array(index_y_hat)
    ...
    def kvret_actions(self):
        navigate = ['action_find_place', 'action_report_address','action_report_distance',
                    'action_set_navigation']#,
        weather = [ 'action_check_weather']
        schedule = ['action_report_event', 'action_set_reminder']
        general = [ 'action_morehelp', 'action_goodbye','action_welcome']#'action_ok',
        all_action = navigate + weather + schedule + general
        return all_action

    def kvret_actions_old(self):
    ...
    def get_full_word2index(self,top_n=10000):
        embedding_index = self.get_glove_dict()
        if top_n:
            print("get first ",top_n," word")
    ...
    def get_best_action(self, action_prop_dict,allow_action =None):
        best_action = None
        max_p = 0
        for action, p in action_prop_dict.items():
            general = ['action_morehelp', 'action_goodbye', 'action_welcome']
            if allow_action:
...
class Glove(DuelRnnCamRest):
    def default_config(self):
        ...
        self.allow_action = self.all_action
        self.word2index = self.get_full_word2index(top_n=10000)[0]
        # self.small_intent_classifier = IntentModel()
        # self.small_intent_classifier.load_model_by_name("act_kvret")

    def clean_labels(self,labels):
        labels = [l.strip() for l in labels]
        if 'action_find_place' in labels:
            labels = ['action_find_place']
        elif 'action_set_navigation' in labels:
            labels = ['action_set_navigation']
        elif 'action_report_distance' in labels:
            labels = ['action_report_distance']
        elif 'action_report_address' in labels:
            labels = ['action_report_address']
        elif "action_check_weather" in labels:
            labels = ["action_check_weather"]
        elif "action_clearify" in labels:
            labels = ["action_check_weather"]
        elif "action_ask_location" in labels:
            labels = ["action_check_weather"]
        elif "action_ask_newevent_time" in labels:
            labels = ["action_set_reminder"]
        return labels

    def krvet_keywords(self):
        # return ["set","reminder","near","address","traffic",\
        #        "weather","direction","search","route","schedule"]
        return ["navigation"]
    ...
    def prepare_krvet(self):
        ...
        intent_classifier = IntentModel()
        intent_classifier.load_model_by_name("act_kvret")
        ...
        for row in train_data:
            history,labels,intent,last_labels,slot_vector = row
            intent_vec = [0]*3
            intent_vec[["navigate","schedule","weather"].index(intent)] = 1
            small_intent_vec = intent_classifier.get_intent_vec(history[-2])
            intent_vec += small_intent_vec
            ...
            unit_feature = []
            map_dict = {"intent":intent_vec,"lastact":y_last,"keyword":keyword_feature,"slotfilling_state":slot_vector}
            for feature in feature_names:
                unit_feature += map_dict[feature]
            #unit_feature = intent_vec + y_last + keyword_feature + slot_vector
            x_features.append(unit_feature)
    ...
    def get_utterance(self, history):
        ...
        for i,turn in enumerate(history):
            if i% 100 ==0:
                print("start processing turn",i)
            turn_vector = []
            for utterance in turn:
                seqs = self.utter_vec(utterance)
                turn_vector += seqs
            turn_vector = self.fix_length_vector(turn_vector, self.MAX_SENTENCE_LENGTH * self.turn_number)
            his_data.append(turn_vector)
            his_i += 1
    ...
    def map_feature_length(self):
        map_dict = {"intent": 3+len(small_intent_for_kvret)+6, "lastact": len(self.kvret_actions()), "keyword": 2 * len(self.krvet_keywords()),
                    "slotfilling_state": len(useful_slots)}
        return map_dict

    def build_binary_model(self,is_binary=True,add_last_action=False,features= feature_names):
        n_class = len(self.all_action)
        word_index,vocab_size,embedding_index = self.get_full_word2index()
        ...
        sentence_level_rnn = Dense(final_layer_size,name="sentence_level_rnn2final_layer_size")(sentence_level_rnn)
        feature_num =0
        map_dict = self.map_feature_length()
        for feature in features:
            feature_num += map_dict[feature]
        print("feature ",feature,"length",feature_num)
        x_feature = Input(shape=(feature_num,), name="_".join(features))
        coef = Dense(final_layer_size,name="x_intent2final_layer_size")(x_feature)
        ...
        multi_class_layer = Dense(len(self.all_action), activation='softmax')(sentence_level_rnn)
        binary = Dense(1, activation='sigmoid')(sentence_level_rnn)
        inputs = x_utterance_concat
        #if has_intent:
        inputs = [x_utterance_concat, x_feature]
        if add_last_action:
    ...
    def get_next_action_from_utters_intent_lastact_slot_state(self,utters, intent,lastact,slotfilling):
        vec = self.array_from_utters(utters)
        intent_vec = [0] * 3
        intent_vec[["navigate", "schedule", "weather"].index(intent)] = 1
        u =utters[-1]
        small_vec = self.small_intent_classifier.get_intent_vec(u)
        intent_vec += small_vec
        intent_vec = np.array(intent_vec)
        act_vec = self.one_hot(lastact,self.kvret_actions())
        slot_vec = HelperNER.entity2vector(slotfilling)
        cb= intent_vec.tolist()+act_vec.tolist()+self.onehot_for_utters(utters[:-1])+slot_vec
        cb = np.array(cb)
        cb = np.expand_dims(cb, 0)
        multi_action_prop_dict = self.predict_one([vec,cb], False)
        allowed_action = get_valid_action_from_intent(intent)
        best_action = self.get_best_action(multi_action_prop_dict,allowed_action)
        return best_action
...
def evaluate():
    g = Glove()
    g.show_paras()
    g.model_path = "C://model/pretrain_kvret_small_intent2.h5"
    g.all_action = g.kvret_actions()
...
def train():
    ...
    g.model.fit([Xtrain,intent_train], ytrain, batch_size=g.batch_size, verbose=2,
                   epochs=5,
                   validation_data=([Xtest,intent_test], ytest),callbacks=[tb])
